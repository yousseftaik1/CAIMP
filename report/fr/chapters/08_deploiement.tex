...
\paragraph{Étape 2 — Configurer les variables d'environnement}
\begin{lstlisting}[language=YAML]
cp .env.example .env
# Editer .env : renseigner les mots de passe et JWT_SECRET
# JWT_SECRET : openssl rand -hex 64
\end{lstlisting}

\paragraph{Étape 3 — Démarrer la plateforme}
\begin{lstlisting}[language=YAML]
make up        # Build des images, migrations BD, démarrage des 19 services
               # (télécharge Ollama depuis Docker Hub automatiquement)
make ps        # Vérifier que tout est "healthy"
\end{lstlisting}

\noindent \texttt{make up} télécharge automatiquement toutes les images tierces
(Ollama, PostgreSQL, Redis, NATS, Grafana, Prometheus, nginx) depuis Docker Hub
lors de la première exécution.

\paragraph{Étape 4 — Télécharger les modèles \textsc{ia}} (première exécution uniquement)
\begin{lstlisting}[language=YAML]
make pull-models   # llama3.1:8b-instruct-q4_K_M (~4.7 Go)
                   # + nomic-embed-text (~270 Mo)
\end{lstlisting}

\paragraph{Étape 5 — Accéder au tableau de bord}
Ouvrir \texttt{http://localhost} dans un navigateur.\\
Identifiants par défaut : \texttt{admin@caimp.local} / \texttt{Admin1234!}

\paragraph{Étape 6 — Enrôler un serveur cible}
Générer un jeton d'enrôlement via l'\api{} Admin et exécuter le programme
d'installation sur la machine cible (voir Section~\ref{sec:surveillance-vm}).
