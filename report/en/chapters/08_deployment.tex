...
\begin{enumerate}
  \item \textbf{Clone the repository:}
    \texttt{git clone https://github.com/your-org/caimp.git \&\& cd caimp}
  \item \textbf{Configure environment variables:}
    \texttt{cp .env.example .env} — edit passwords and generate JWT\_SECRET
    (\texttt{openssl rand -hex 64})
  \item \textbf{Start the platform:}
    \texttt{make up} — builds all images, runs migrations, starts all 19 services
  \item \textbf{Pull AI models} (first run only):
    \texttt{make pull-models} — downloads llama3.1:8b and nomic-embed-text
  \item \textbf{Open the dashboard:} \texttt{http://localhost}\\
    Default credentials: \texttt{admin@caimp.local} / \texttt{Admin1234!}
  \item \textbf{Enrol a target server:} generate an enrollment token via the
    Admin API, then run the agent on the target server.
\end{enumerate}
