...
async def list_orgs(user=Depends(require_admin)):
    ...
    return [
        OrgResponse(
            id=str(r["id"]), name=r["name"], plan=r["plan"], created_at=r["created_at"]
        )
        for r in rows
    ]
...
async def get_org(org_id: str, user=Depends(get_current_user)):
    ...
    async with get_conn() as conn:
        row = await conn.fetchrow(
            "SELECT id, name, plan, created_at FROM organizations WHERE id = $1::uuid",
            org_id,
        )
    if not row:
        raise HTTPException(
...
async def delete_org(org_id: str, user=Depends(require_admin)):
    async with get_conn() as conn:
        result = await conn.execute(
            "DELETE FROM organizations WHERE id = $1::uuid", org_id
        )
    if result == "DELETE 0":
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
