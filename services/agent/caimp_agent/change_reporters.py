...
class ChangeReporter:
    ...
    async def _post(self, change_type: str, source: str, actor: str | None,
                    description: str, payload: dict,
                    image_sha: str | None = None) -> None:
        if not self._http:
        ...
        while True:
            try:
                ...
                async for line in proc.stdout:
                    ...
                    if etype not in ("container", "image") or action not in self._WATCHED:
                        continue
                    ...
                    await self._post(
                        change_type=change_type, source="docker_events",
                        actor="docker", description=desc,
                        payload={"action": action, "image": image, "container": name},
                        image_sha=sha,
                    )

            except FileNotFoundError:
                log.debug("Docker not available on this server — skipping docker events reporter")
                return
            except Exception as exc:
        ...
        try:
            ...
            async for line in proc.stdout:
                ...
                if m:
                    ...
                    await self._post(
                        change_type="ssh_login", source="auth_log",
                        actor=user,
        ...
        try:
            ...
            async for line in proc.stdout:
                ...
                if m:
                    cmd = m.group(1)[:120]
                    await self._post(
                        change_type="cron", source="syslog",
                        actor="cron",
...
async def start_all_reporters(cfg: "AgentConfig", server_id: str, jwt_token: str) -> None:
