...
def collect() -> list[dict]:
    ...
    try:
        ...
        records.append({
            ...
            "resource_attributes": resource,
            "data_points": [{"value": disk_pct / 100.0, "time_unix_nano": now_ns,
...
def send(records: list[dict]) -> None:
    ...
    try:
        ...
        if resp.ok:
            cpu_val = next((r["data_points"][0]["value"] for r in records
                            if r["name"] == "system.cpu.utilization"), 0)
            mem_val = next((r["data_points"][0]["value"] for r in records
                            if r["name"] == "system.memory.utilization"), 0)
            suffix = " [OVERRIDE]" if CPU_OVERRIDE > 0 or RAM_OVERRIDE > 0 else ""
            log.info("Sent %d metrics  CPU=%.1f%%  RAM=%.1f%%%s",
                     len(records), cpu_val * 100, mem_val * 100, suffix)
        else:
            log.warning("Telemetry writer returned %s: %s", resp.status_code, resp.text[:200])
    except requests.exceptions.ConnectionError:
        log.error("Cannot reach telemetry-writer at %s — is docker-compose running?", url)
    except Exception as exc:
        log.error("Send failed: %s", exc)


def main() -> None:
    log.info("Demo agent starting — server=%s  interval=%ds  endpoint=%s",
             SERVER_NAME, INTERVAL, TELEMETRY_URL)
