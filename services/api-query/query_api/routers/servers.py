...
async def server_summary(server_id: str, user=Depends(get_current_user)):
    ...
    async with get_tenant_conn(user.org_id) as conn:
        srv = await conn.fetchrow(
            "SELECT id, name, hostname, status FROM servers WHERE id = $1::uuid", server_id
        )
        if not srv:
        ...
        anomaly_count = await conn.fetchval(
            ...
            user.org_id, server_id, since_24h,
        )
    ...
    await set_cached(result, "srv_summary", ttl_seconds=30,
                     org_id=user.org_id, server_id=server_id)
    return result
