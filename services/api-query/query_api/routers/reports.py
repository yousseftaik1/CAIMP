...
def _build_pdf(data: dict) -> bytes:
    ...
    SMALL = _ps("body_small",  fontSize=8,   textColor=DGRY,  leading=11)
    NARR  = _ps("body_narr",   fontSize=8.5, textColor=BRAND, fontName="Helvetica-Oblique", leading=13)
    ...
    def _section(title: str, subtitle: str = "") -> list:
        ...
        items: list = [Table(
            [[Paragraph(title, _ps(f"sh_{uid}", fontSize=11, textColor=WHITE,
    ...
    def _status_cell(s: str):
        m = {"ok": ("Healthy", H_GREEN), "warning": ("Watch", H_ORANGE),
    ...
    def _trend_cell(slope: float):
        ...
        if slope < -0.001:
            return Paragraph(f'<font color="{H_GREEN}"><b>Easing down</b></font>', CELLC)
        return Paragraph(f'<font color="{H_DGRY}">Stable</font>', CELLC)

    def _anomaly_val(v: float, metric: str) -> str:
        frac = {"cpu_percent", "memory_percent", "disk_percent",
    ...
    def _cover_chrome(canvas, doc):
        ...
        def _lx(x): return ix + x * sc
        def _ly(y): return iy + (36 - y) * sc   # SVG y↓ → PDF y↑
        ...
        canvas.setFillColor(ACCENT)
        canvas.drawString(MARGIN, PH * 0.475 - 25 * mm,
        ...
        canvas.drawString(MARGIN, my - 15 * mm,  f"Servers         {data['server_count']} monitored")
        # Bottom strip
        ...
        canvas.drawCentredString(PW / 2, 6 * mm,
    ...
    def _page_chrome(canvas, doc):
        ...
        def _hx(x): return mox + x * ms
        def _hy(y): return moy + (36 - y) * ms
        ...
        canvas.setFillColor(WHITE)
        canvas.drawString(bx + bs + 3 * mm, PH - 8 * mm,
                          "CAIMP  ·  Infrastructure Health Report")
        ...
        canvas.drawCentredString(PW / 2, 8 * mm,
                                 f"Page {doc.page - 1}  —  Confidential")
        canvas.restoreState()
    ...
    def _kpi(label: str, value, sub: str = "", hcolor: str = H_BRAND) -> Table:
        ...
        rows: list = [
            [Paragraph(str(value),
                       _ps(f"kv{n}", fontSize=22, fontName="Helvetica-Bold", leading=28,
                           textColor=colors.HexColor(hcolor), alignment=TA_CENTER))],
            [Paragraph(label,
                       _ps(f"kl{n}", fontSize=8, textColor=DGRY, leading=11,
                           alignment=TA_CENTER))],
        ]
        if sub:
            rows.append([Paragraph(
                sub, _ps(f"ks{n}", fontSize=7, leading=10, alignment=TA_CENTER,
                         textColor=colors.HexColor(hcolor)))])
        return Table(rows, colWidths=[CW / 4 - 3],
                     style=TableStyle([
    ...
    story.append(Table(
        [[
            ...
            _kpi("Issues Detected",    s["anomaly_count"],
                 "Need attention" if s["anomaly_count"] > 0 else "All clear", an_col),
            _kpi("Urgent Forecasts",   s["critical_forecast_count"],
                 "At risk soon" if s["critical_forecast_count"] > 0 else "Looking good", fc_col),
            _kpi("AI Recommendations", s["ai_count"],
    ...
    if data["forecasts"]:
        ...
        crit_fcs = [fc for fc in data["forecasts"]
        ...
        for fc in data["forecasts"]:
            ...
            if ttc is not None:
                h = int(ttc)
                ttc_str = ("Less than 1 hour" if h <= 1
                           else f"About {h} hours" if h <= 6
                           else f"~{h} hours away")
            ...
            conf_hx = H_GREEN if conf == "high" else (H_ORANGE if conf == "medium" else H_DGRY)
            ...
            fc_rows.append([
                ...
                Paragraph(f'<font color="{conf_hx}"><b>{conf.capitalize()}</b></font>', CELLC),
            ])

        fc_cw = [CW * r for r in (0.17, 0.11, 0.14, 0.11, 0.12, 0.21, 0.14)]
        story.append(Table(
            fc_rows, colWidths=fc_cw,
            style=TableStyle(_BASE + _HDR + _STRIPE),
            repeatRows=1,
        ))
        story.append(Spacer(1, 6 * mm))
                ...
                story.append(Table(
                    [[
                        Paragraph(
                            ...
                            _ps(f"nh{i}", fontSize=8.5, textColor=WHITE,
                                fontName="Helvetica-Bold", leading=13),
                        ),
                        Paragraph(
                            f'<font color="{conf_hx}">{conf.capitalize()} reliability</font>',
                            _ps(f"nc{i}", fontSize=8, textColor=ACCENT,
    ...
    if data["ai_insights"]:
        ...
        for idx, ins in enumerate(data["ai_insights"]):
            ...
            conf_hx = H_GREEN  if ins["confidence"] == "high"     else (
                      H_ORANGE if ins["confidence"] == "medium"   else H_DGRY)

            story.append(Table(
                [[
                    Paragraph(
                        f'{ins["server_name"]}  —  {_label(ins["anomaly_type"])}',
                        _ps(f"ih{idx}", fontSize=9, textColor=WHITE,
                            fontName="Helvetica-Bold", leading=13),
                    ),
                    Paragraph(
                        ...
                        _ps(f"im{idx}", fontSize=8, textColor=ACCENT,
                            fontName="Helvetica", leading=13, alignment=TA_RIGHT),
                    ),
                ]],
            ...
            if ins.get("explanation"):
                detail_rows.append([
                    ...
                    Paragraph(ins["explanation"], _ps(f"ed{idx}", fontSize=8.5, leading=13)),
                ])
            if ins.get("root_cause"):
                detail_rows.append([
                    Paragraph("Why it happened",  CELLB),
                    Paragraph(ins["root_cause"],   _ps(f"rd{idx}", fontSize=8.5, leading=13)),
                ])
            if ins.get("recommended_action"):
                detail_rows.append([
                    Paragraph("What to do",        CELLB),
                    Paragraph(ins["recommended_action"],
                               _ps(f"ad{idx}", fontSize=8.5, leading=13,
...
async def _collect(conn, org_id: str, t_from: datetime, t_to: datetime,
    ...
    srv_metrics: dict[str, dict] = {
        sid: {"cpu_pct": None, "ram_pct": None, "disk_pct": None, "anomaly_count_24h": 0}
    ...
    if server_ids:
        ...
        _key_map = {"cpu_percent": "cpu_pct", "memory_percent": "ram_pct",
    ...
    servers = [
        {"id": str(r["id"]), "name": r["name"], "hostname": r["hostname"],
         "status": r["status"], **srv_metrics.get(str(r["id"]), {})}
    ...
    for fr in forecast_rows:
        ...
        pts = pts or []
        forecasts.append({
            "server_name":      server_map.get(str(fr["server_id"]), str(fr["server_id"])),
    ...
    critical_fc = sum(1 for f in forecasts
                      if f.get("time_to_critical") and f["time_to_critical"] <= 6)
...
async def download_pdf(
    from_time: datetime | None = Query(default=None,
                                       description="Start of report window (default: 24 h ago)"),
    to_time:   datetime | None = Query(default=None,
                                       description="End of report window (default: now)"),
    server_id: str | None      = Query(default=None,
                                       description="Filter report to a single server UUID"),
    ...
    filename = (
        f"caimp-report-{t_from.strftime('%Y%m%d')}"
        f"-to-{t_to.strftime('%Y%m%d')}.pdf"
    )
