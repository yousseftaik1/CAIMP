...
async def list_incidents(
    ...
    for r in rows:
        ...
        result.append(IncidentSummary(
            ...
            value=round(float(r["value"]) * 100, 1) if float(r["value"]) <= 1.0 else round(float(r["value"]), 1),
...
async def get_incident(incident_id: str, user=Depends(get_current_user)):
    ...
    if row["ai_id"]:
        ai = AIExplanation(
            ...
            model=row["model"],
            created_at=row["ai_created_at"].isoformat() if row["ai_created_at"] else None,
        )
...
async def submit_feedback(
    ...
    async with get_tenant_conn(user.org_id) as conn:
        ai = await conn.fetchrow(
            ...
            incident_id, user.org_id,
        )
        if not ai:
            raise HTTPException(status_code=404, detail="No AI explanation for this incident")
        await conn.execute(
            """INSERT INTO ai_feedback (explanation_id, org_id, rating, comment)
               VALUES ($1::uuid, $2::uuid, $3, $4)
               ON CONFLICT DO NOTHING""",
            ai["id"], user.org_id, body.rating, body.comment,
        )
...
async def list_runbooks(
    ...
    return [RunbookSummary(
        id=str(r["id"]), title=r["title"], anomaly_type=r["anomaly_type"],
...
async def get_runbook(runbook_id: str, user=Depends(get_current_user)):
    async with get_tenant_conn(user.org_id) as conn:
        r = await conn.fetchrow(
            ...
            user.org_id, runbook_id,
        )
    if not r:
        raise HTTPException(status_code=404, detail="Runbook not found")
    return RunbookDetail(
        id=str(r["id"]), title=r["title"], anomaly_type=r["anomaly_type"],
        content=r["content"], created_by=str(r["created_by"]) if r["created_by"] else None,
        created_at=r["created_at"].isoformat(), updated_at=r["updated_at"].isoformat(),
        has_embedding=r["has_embedding"],
    )
...
async def _embed(text: str) -> list[float] | None:
    try:
        async with httpx.AsyncClient(timeout=30) as http:
            resp = await http.post(f"{settings.ollama_url}/api/embeddings",
...
async def create_runbook(body: RunbookCreate, user=Depends(get_current_user)):
    ...
    async with get_tenant_conn(user.org_id) as conn:
        r = await conn.fetchrow(
            ...
            user.org_id, body.title, body.anomaly_type, body.content,
            str(embedding) if embedding else None,
        )
    return RunbookDetail(
        id=str(r["id"]), title=r["title"], anomaly_type=r["anomaly_type"],
        content=r["content"], created_by=None,
        created_at=r["created_at"].isoformat(), updated_at=r["updated_at"].isoformat(),
        has_embedding=r["has_embedding"],
    )
...
async def update_runbook(runbook_id: str, body: RunbookUpdate, user=Depends(get_current_user)):
    async with get_tenant_conn(user.org_id) as conn:
        ...
        r = await conn.fetchrow(
            """UPDATE runbooks SET title=$1, anomaly_type=$2, content=$3, embedding=$4::vector,
               ...
               RETURNING id, title, anomaly_type, content, created_by, created_at, updated_at,
                         (embedding IS NOT NULL) AS has_embedding""",
            new_title, new_type, new_content,
            str(embedding) if embedding else None,
            runbook_id, user.org_id,
        )
    return RunbookDetail(
        id=str(r["id"]), title=r["title"], anomaly_type=r["anomaly_type"],
        content=r["content"], created_by=str(r["created_by"]) if r["created_by"] else None,
